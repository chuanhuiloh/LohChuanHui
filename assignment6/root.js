  ...
  return (
    <div className="blog-list">
      {props.posts.map(post => (
        post.private ?
        <PrivateBlogPost title={post.title} author={post.author} date={post.date}/> :
        <PublicBlogPost title={post.title} author={post.author} date={post.date} content={post.content} />))}
    </div>
  )
...
function Footer(props) {
  return (
    <>
      <footer>&copy; {props.currentYear} My Blog. All rights reserved.</footer>
    </>
  )
};
